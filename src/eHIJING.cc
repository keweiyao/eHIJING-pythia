...
double PhiG(double x, double q2, double Qs2, double Q2x, double powerG, double lambdaG, double Kfactor_){
    double result;
    ...
    return result * Kfactor_/((Q2x>Qs2)? (1+std::log(Q2x/Qs2)) : (Q2x/Qs2));
}
double alphas_PhiG(double x, double q2, double Qs2, double Q2x, double powerG, double lambdaG, double Kfactor_){
    double result;
...
double MultipleCollision::Qs2_self_consistent_eq(double Qs2, double TA, double Q2x){
    ...
    auto dfdq2 = [this, Qs2, Q2x](double ln1_q2oQs2) {
        ...
        double x = q2/Q2x;
        return alphas_PhiG(x, q2, Qs2, Q2x, this->powerG_, this->lambdaG_, this->Kfactor_) * Jacobian;
    };
    double error;
    double res =  scaledTA * quad_1d(dfdq2, {std::log(1+.01*mu2/Qs2), std::log(1+Q2x/Qs2)}, error);
    return res - Qs2;
}
...
int MultipleCollision::sample_all_qt2(int pid, double E, double L, double xB, double Q2,
    ...
    double tildeTA = Kfactor_/((q2max>qs2)?(1+std::log(q2max/qs2)):(q2max/qs2))*M_PI*CR*TA/dA;
    if (tildeTA<1e-15) return q2_list.size();
    double qs2overCTA = qs2/tildeTA;
    double q2 = q2max;
    double q2min = EHIJING::mu2*.01;
    while (q2 > q2min) {
