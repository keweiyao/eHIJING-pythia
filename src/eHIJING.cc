...
GHT_z_kt2_Table(4, {11,51,51,201}, // TA, ln(Q2/x), ln(kt2), ln(3+L/tauf) --> size and grid for H-T table -- 1
           {TAmin, std::log(1.0), std::log(2*mu2), std::log(5+.05)}, 
           {TAmax, std::log(1e2), std::log(1e2),  std::log(5+500)}
       ),
GHT_kt2_Table(4, {11,51,51,201}, // TA, ln(Q2/x), ln(kt2), ln(3+L*kt2/2E) --> size and grid for H-T table -- 2
           {TAmin, std::log(1.0), std::log(2*mu2), std::log(5+.05)}, 
           {TAmax, std::log(1e2), std::log(1e2),  std::log(5+500)}
       )
{
...
void eHIJING::Tabulate(std::filesystem::path table_path){
    ...
    if (mode_==1) {
        ...
        std::cout << "... All tables done" << std::endl;
...
double eHIJING::Qs2_self_consistent_eq(double Qs2, double TA, double Q2x){
    ...
    double error;
    double res =  normG(Q2x, Qs2) * scaledTA * quad_1d(dfdq2, {std::log(1+mu2/Qs2), std::log(1+Q2x/Qs2)}, error);
    return res - Qs2;
}
...
void eHIJING::sample_all_qt2(int pid, double E, double L, double xB, double Q2,
    ...
    double tildeTA = Kfactor_*normG(q2max, qs2)*M_PI*CR*TA/dA;
    ...
    while (q2 > mu2) {
        ...
        if (q2 < qs2) {
            q2 = q2*std::pow(1.0 + lambdaG*lnr*qs2overCTA*std::pow(q2max/q2, lambdaG),
                             1./lambdaG);
        }
        else{
            double Pc = tildeTA/(lambdaG-1.)*( std::pow(q2/q2max, lambdaG) / q2
                                             - std::pow(qs2/q2max, lambdaG) / qs2
                                             );
            if (lnr > -Pc) {
                q2 = q2 * std::pow(1.0 + (lambdaG-1.)*lnr*q2/tildeTA*std::pow(q2max/q2, lambdaG),
                                   1./(lambdaG-1.));
            }
            else {
                q2 = qs2 * std::pow(1.0 + lambdaG*(lnr+Pc)*qs2overCTA*std::pow(q2max/qs2, lambdaG),
                                   1./lambdaG);
            }
        }
        double xg = q2/q2max;
        if (q2 > mu2 && flat_gen(gen) < std::pow(1.-xg, powerG)) {
...
bool eHIJING::next_kt2(double & kt2, int pid, double E, double L, 
    ...
    if (mode_==0) { // Collinear HT 
        ...
        double kt2_c = 2*std::max(Kfactor_*2*M_PI*qs2 * 2*(CHT_F2(Mproton_*L, zmin)-CHT_F2(Mproton_*L, zmax)) / logvac, 4*mu2);
        ...
        while (acceptance<flat_gen(gen) && kt2>kt2min) {
            ...
            if ( kt2_c < kt2 || Kfactor_ < 1e-3) {
                ...
                acceptance = ( 1 + Kfactor_*induced_dFdkt2(xB, Q20, L, E, kt2) /logvac)/(1.0+4*Kfactor_*L*qs);
...
bool eHIJING::sample_z(double & z, int pid, double E, double L, double kt2, double xB, double Q20) {
    ...
    return true;
     /*(acceptance * (1.0 + (1.0-z/2.)*W0 ) / (1.0+W0) > flat_gen(gen)) 
        && (z*(1-z)>cut)
        && (std::pow(1-z, 2)>cut)
        && (std::pow(z,2)>cut)
        && (z*E > 0.5)
