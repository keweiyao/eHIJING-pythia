...
double PhiG(double x, double q2, double Qs2, double Q2x, 
...
double alphas_PhiG(double x, double q2, double Qs2,
		   double Q2x, double powerG, double lambdaG, double K){
    double result;
...
void MultipleCollision::Tabulate(std::filesystem::path table_path){
    ...
    if (std::filesystem::exists(fname)) {
        ...
        for (int c=0; c<Qs2Table.size(); c++) {
            ...
            double aQs2 = compute_Qs2(TA, Q2xB);
...
double MultipleCollision::Qs2_self_consistent_eq(double Qs2, double TA, double Q2x){
    ...
    auto dfdq2 = [this, Qs2, Q2x](double ln1_q2oQs2) {
        ...
        double x = q2/Q2x;
        return alphas_PhiG(x, q2, Qs2, Q2x, 
			this->powerG_, this->lambdaG_, this->Kfactor_) * Jacobian;
    };
    double error;
    double res =  scaledTA * quad_1d(dfdq2, {std::log(1+.001*mu2/Qs2), std::log(1+Q2x/Qs2)}, error);
    return res - Qs2;
}
