...
// WK>>>
void Event::fill_elastic_collision(Particle & p){
  // Currently deactiavted
  /*p.ResetMultipleCollision();
  std::uniform_real_distribution<double> uniform(0.,1.);
  double CR = 3.; // note that for the soft collisions for induced radiations 
                  // are mainly gluon!
  double effcouplingcoefficient = 3;
  double mu2 = pow2(1.0);
  double Qmax = 4.0;
  double Q2max = pow2(Qmax);
  int absid = p.idAbs();
  // Nucleus beam momentum
  double vAz = entry[1].pz()/entry[1].e();
  // get the particle momentum in the A rest frame
  Vec4 pinA = p.p();
  pinA.bst(0., 0., -vAz);
  double ELab_over_EA = p.e()/pinA.e();
  double vx = pinA.px()/pinA.pAbs(),
         vy = pinA.py()/pinA.pAbs(),
         vz = pinA.pz()/pinA.pAbs();
  double xdotv = p.Rx()*vx + p.Ry()*vy + p.Rz()*vz;
  double R2 = pow2(3.7*5.076), x2 = pow2(p.Rx())+pow2(p.Ry())+pow2(p.Rz());
  double v2 = vx*vx + vy*vy + vz*vz;
  if (R2<x2) return;
  double L = (-xdotv + std::sqrt(v2*(R2-x2)+xdotv*xdotv) ) / v2;
  //if( p.idAbs()==2 || p.idAbs()==1)
  //p.p(pIn);
  double sigmapN = CR*effcouplingcoefficient*(1./mu2 - 1./Q2max);
  double rho0 = 0.0013; // GeV^3
  
  std::vector<double> ts; ts.clear();
  std::vector<double> q2s; q2s.clear();
  std::vector<double> phis; phis.clear();

  // Generate q2 ordered collisions
  double q2 = Q2max;
  do{
     double r = uniform(generator);
     q2 = q2/(q2*std::log(1./r)/(rho0*L*CR*effcouplingcoefficient) + 1.);
     if(q2>mu2) {
         q2s.push_back(q2);
         ts.push_back(uniform(generator)*L);
         phis.push_back(uniform(generator)*2*M_PI);
     };
     //std::cout << (q2>mu2) << std::endl;
  }while(q2>mu2);
  int Ncoll = q2s.size();
  ...
  for (int i=0; i<Ncoll; i++){
      ...
      MultipleCollisionEntry a;    
      Vec4 qmu(q*std::cos(phi),q*std::sin(phi),0.,0.);        
      a.q = qmu;
      a.q2 = q2;
      a.TimeFactor = L;
      p.AddMultipleCollision(a); 
  }*/
}
//WK<<<
//==========================================================================
