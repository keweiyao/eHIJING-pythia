...
void fillPartons(int type, double ee, Event& event, ParticleData& pdt,
  ...
  // Information on a g g g system, to be hadronized.
    ...
    double mm = pdt.m0(id);
    double pp = sqrtpos(ee*ee - mm*mm);
    event.append(  3, 23, 102, 0, 0., 0.,  ee, ee, 0.0);
    event.append( -2, 23,  0, 103, 0., 0., -ee, ee, 0.0);
    event.append( 21, 23, 101, 102, 0., 0.8*pp, 0.6*pp, pp, 0.0);
    event.append( 21, 23, 103, 102, 0., -0.6*pp, -0.8*pp, pp, 0.0);
    event.append( 21, 23, 102, 101, 0., -0.8*pp/2., -0.6*pp/2., pp/2., 0.0);
